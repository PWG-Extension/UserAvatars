...
|	Email:																		|
|	Author:		© Alexander Kadnikoff [Predator]								|
+-------------------------------------------------------------------------------+
|	© «PWG», 2004-2018. All Rights Reserved.									|
+-------------------------------------------------------------------------------+
*/

namespace pwg\UserAvatars\event;
...
class listener implements EventSubscriberInterface
{
	...
	const MAX_SIZE = 24; // Max size img
	...
	*/
	...
	public function viewforum_last_post_avatar($event)
	{
		...
		if (!empty($this->userrow[$row['topic_last_poster_id']]))
		{
			...
			$topic_row['LAST_POST_AUTHOR_FULL'] .= '<span class="lastpostavatar"><a href="'.$topic_row['U_LAST_POST_AUTHOR'].'">'.phpbb_get_user_avatar($userrow).'</a></span>'; # Predator
	...
	public function search_last_post_avatar($event)
	{
		...
		{
			$row = $this->avatar_img_resize($row);
			$tpl_ary['LAST_POST_AUTHOR_FULL'] .= '<span class="lastpostavatar"><a href="'.$topic_row['U_LAST_POST_AUTHOR'].'">'.phpbb_get_user_avatar($row).'</a></span>'; # Predator
